...
describe("TESTS FOR KING", () => {
  ...
  test("KING MOVE", () => {
    ...
    const moves: Square[] = whiteKing.validMoves();

    moves.forEach(move => {
      expect(newGame.isMovePossible(
        new Move(new Square(5, 2), new Square(5 + move.row, 2 + move.column), whiteKing, null)))
        .toBe(true);
    });
  ...
  test("KING CANNOT MOVE TO ITSELF POSITION", () => {
    ...
    newBoard.addPiece(whiteKing);
    const newGame = new Game(undefined, PieceColor.White, undefined, undefined, undefined, newBoard);
    const move = new Move(new Square(5, 2), new Square(5, 2), whiteKing, null);
    expect(newGame.isMovePossible(move)).toBe(false);
  });
